########################################################################################################################################
#
# skrypt pobiera dane na temat uzytkownikow bazy ldap z pliku roboczego JSON i wyswietla informacje o uzytkownikach ktorych konta sa
# wylaczone wiecej niz < n > dni. Zmienna < n > ustawiamy w zaleznosci od wytycznych w dokumentacji systemu.
# 
...
########################################################################################################################################
$days = 366 # tu podac ilosc dni powyzej ktorych ma sprawdzac (366 dni ~ 1 rok)
...
if(fileValidation($file) -eq $true){
    Write-Host "*************************************************************************************************************"
    Write-Host "KONTA WYLACZONE, ostatnie logowanie w domenie " -ForegroundColor Gray -NoNewline
    ...
    foreach ($user in $allUsersFromjsonBaseWorkFile){
        ...
        if (userValidation($principalName) -eq $true) { # to wyklucza konta techniczne
            ...
            if ($accountStatus -eq $true){ # to daje tylko aktywne konta
